...
# 						They are required for checking if the scanning phase has finished correctly.
	...
	def loadUserConfig(self, userConfigPath):
		...
		if not config["target_drive_full_action"] in DRIVE_FULL_ACTIONS:
			...
			config["target_drive_full_action"] = DRIVE_FULL_ABORT
	...
	def checkFreeSpace(self):
		...
		if (freeSpace < stats.bytes_to_copy):
			if self.config["target_drive_full_action"] == DRIVE_FULL_PROMPT:
				answer = ''
				...
				if answer.lower() == 'n':
					...
					raise BackupError
			elif self.config["target_drive_full_action"] == DRIVE_FULL_ABORT:
				logging.critical("The target drive has %s free space. The backup is expected to need another %s. The backup will be aborted."
								 % (sizeof_fmt(freeSpace), sizeof_fmt(stats.bytes_to_copy)))
				raise BackupError
			elif self.config["target_drive_full_action"] == DRIVE_FULL_PROCEED:

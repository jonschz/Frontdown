...
class statistics_module:
    ...
    def rows(cls, data: list[Optional[tuple[str, str]]]) -> str:
        ...
        def formatRow(row: tuple[str, str]) -> str:
            return (
                (" " * cls.INDENT) + (row[0] + ":").ljust(cls.LABEL_WIDTH, " ") + row[1]
            )
    ...
    def scanning_protocol(self) -> str:
        return self.rows(
            [
                (
                    "Source",
                    f"{self.folders_in_source} folders, {self.files_in_source} files, {sizeof_fmt(self.bytes_in_source)}",
                ),
                (
                    "Compare",
                    f"{self.folders_in_compare} folders, {self.files_in_compare} files, {sizeof_fmt(self.bytes_in_compare)}",
                ),
    ...
    def action_generation_protocol(self) -> str:
        return self.rows(
            [
                (
                    "To copy",
                    f"{self.files_to_copy} files, {sizeof_fmt(self.bytes_to_copy)}",
                ),
                (
                    "To hardlink",
                    f"{self.files_to_hardlink} files, {sizeof_fmt(self.bytes_to_hardlink)}",
                ),
                (
                    None
                    if self.files_to_delete == 0
                        ...
                        "To delete",
                        f"{self.files_to_delete} files, {sizeof_fmt(self.bytes_to_delete)}",
                    )
                ),
            ]
        )
    ...
    def backup_protocol(self) -> str:
        return self.rows(
            [
                (
                    "Copied",
                    f"{self.files_copied} files, {sizeof_fmt(self.bytes_copied)}",
                ),
                (
                    "Hardlinked",
                    f"{self.files_hardlinked} files, {sizeof_fmt(self.bytes_hardlinked)}",
                ),
                ...
                (
                    ...
                    if self.files_to_delete == 0
                        ...
                        "Deleted",
                        f"{self.files_deleted} files, {sizeof_fmt(self.bytes_deleted)}",
                    )
                ),
    ...
    def scanningError(
        self, errorMsg: str, exc_info: BaseException | None = None
    ) -> None:
